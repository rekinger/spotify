...
export function Artist({ name, images, id }: { name: string, images: Image[], id: string }) {
    ...
    return (
        <Button className="flex flex-col w-full h-full bg-transparent rounded-none">
            <Link href={`/artists/${id}`} className="artist flex items-center justify-center h-full w-full">
