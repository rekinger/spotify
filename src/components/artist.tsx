...
export function Artist({ name, images, id }: { name: string, images: Image[], id: string }) {
    ...
    return (
        <Link href={`/artists/${id}`} className="artist flex flex-col w-full h-full justify-center items-center">
            <motion.div layout="position" transition={{duration:0.5}} className="flex flex-col w-full h-full justify-center items-center">
                <Button className="p-0 h-28 rounded-none w-28 md:w-32 md:h-32 lg:w-44 lg:h-44">
                    <div className="relative h-28 w-28 md:w-32 md:h-32 lg:w-44 lg:h-44">
                        ...
                        <NextImage onLoad={() => {setImageLoaded(true)}} layout="fill" objectFit="cover" unoptimized alt="Album Image" src={images[0].url} />
                    </div>
                </Button>
